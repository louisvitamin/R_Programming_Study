...
cat("Hit return to see the results of regression model Salary ~ Gender\n")
readline()
fit0 <- lm(Salary ~ Gender, data=bank)
print(summary(fit0))
readline()

cat("Notice that R automatically made Female the reference and Male 
the factor (which can assume values of zero or one).\n")
cat("We can actually check the design matrix used. Hit return to see:\n")
readline()
...
salaries\n")
cat("Intercept: ", coefficients(fit0)[1],"\n")
readline()
...
cat("Male employees on average get", coefficients(fit0)[2], "thousand dollars more than female employees\n")

cat("We can check this information by looking at the means of female
and male salaries separately. Hit return to see:\n")
readline()
...
Age=95-YrBorn.\n")
cat("Hit return to see the new bank data frame\n")
readline()
print(bank)
...
malePoints<-scatterplot3d(bank[bank$Gender=="Male","YrsExper"], 
  ...
  color="blue",pch=16,type="h",xlab="Yrs Exper", ylab="Yrs Prior",
zlab="Salary")
...
femalePoints<-scatterplot3d(bank[bank$Gender=="Female","YrsExper"], 
  ...
  color="red",pch=16,type="h",xlab="Yrs Exper", ylab="Yrs Prior",
zlab="Salary")
...
cat("Hit return to see the males and females together\n")
readline()
# redraw male point without the vertical lines
malePoints<-scatterplot3d(bank[bank$Gender=="Male","YrsExper"], 
  bank[bank$Gender=="Male","YrsPrior"], 
  bank[bank$Gender=="Male","Salary"],
  color="blue",pch=16,xlab="Yrs Exper", ylab="Yrs Prior", zlab="Salary")
...
fit1 <- lm(Salary ~ Gender + YrsExper + YrsPrior, data=bank)
cat("Hit return to see the refined model:\n")
readline()
print(summary(fit1))
readline()
...
significant or not\n")
cat("Hit return to see the results of the partial F-test:\n")
readline()
...
cat("Hit return to see Females' regression plane\n")
readline()
malePoints$plane3d(
  c(coefficients(fit1)[1],
  coefficients(fit1)[3], coefficients(fit1)[4]),col="red",lty="solid")

# Now add the the Males regression plane
cat("Hit return to see Males' regression plane\n")
readline()
malePoints$plane3d(
  c(coefficients(fit1)[1]+coefficients(fit1)[2], 
  coefficients(fit1)[3], coefficients(fit1)[4]),col="blue",lty="solid")
readline()
...
cat("Hit return to see the effect of interaction term\n")
readline()
fit2 <- lm(Salary ~ Gender * YrsExper, data=bank)
print(summary(fit2))
readline()
...
cat("Hit return to see the interaction model for males:\n")
readline()
plot(sort(bank$YrsExper), 30.43 + 1.52 * sort(bank$YrsExper),col="blue",type="l")
cat("Hit return to see the interaction model for females:\n")
readline()
lines(sort(bank$YrsExper),34.53+.28*sort(bank$YrsExper),col="red",type="l")
readline()
...
# Let's see:
cat("Hit return to see regression Salary ~ YrsExper for males:\n")
readline()
print(summary(lm(Salary~YrsExper,data=bank,subset=Gender=="Male")))
readline()
cat("Hit return to see regression Salary ~ YrsExper for females:\n")
readline()
...
cat("Hit return to see the result of the command
...
cat("Hit return to see how \"levels\" function shows levels of EducLev:\n")
readline()
print(levels(bank$EducLev))

cat("Now let's add the education level to the model (Hit return to see
the results)\n")
readline()
fit3 <-lm(Salary ~ Gender + YrsExper + YrsPrior + EducLev, data=bank)
print(summary(fit3))
readline()
...
# of refinements:
cat("Hit return to see the partial F-Test for each of the models:\n")
readline()
print(anova(fit0, fit1, fit3))
readline()
# Note that not all education levels have a significant advantage over
# those only with highschool diploma (the base level).
boxplot(Salary~Gender + EducLev, data=bank)
...
fit4 <- lm(Salary ~ Gender + YrsExper + YrsPrior + EducLev + PCJob, data=bank)
cat("Hit return to see the added effect of PCJob factor\n")
readline()
print(summary(fit4))
readline()
...
fit5 <- lm(Salary~Gender + YrsExper + YrsPrior + EducLev + PCJob +
  JobGrade, data=bank)
cat("Hit return to see the effect of Job grade\n")
readline()
print(summary(fit5))
readline()
...
# bank, males tend to occupy higher grade jobs while females occupy
# lower level ones.
cat("Hit return to see the results of partial F-test\n")
readline()
print(anova(fit0, fit1, fit3, fit5))
...
cat("Hit return to see the matrix of plots\n")
readline()
pairs (Salary~Gender + YrsExper + YrsPrior + EducLev + PCJob + JobGrade,
      data=bank)

# Another possibility, especially for factors id to look at the "boxplots"
# with respect to the response variable. For instance, we can plot salaries
# with respect to gender, with respect to education level, and so on. Again,
# ideally, we should do this *before* building models.
#
cat (" Let's look at some boxplots:\n")
cat(" Hit return to see salary vs gender\n")
readline()
boxplot(Salary ~ Gender, data=bank)

cat(" Hit return to see salary vs education level\n")
readline()
boxplot(Salary ~ EducLev, data=bank)


cat(" Hit return to see salary vs job grade\n")
readline()
boxplot(Salary ~ JobGrade, data=bank)

cat(" Hit return to see salary vs PC job\n")
readline()
